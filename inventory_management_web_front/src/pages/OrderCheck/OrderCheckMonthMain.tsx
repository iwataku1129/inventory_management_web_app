...
export const DashboardContentMonthMain = (props: { month: string, notCompflg: boolean, supplierCode: string }) => {
    ...
    useEffect(() => {
        ...
        if (inProgress === InteractionStatus.None) {
            ApiGet(apiPath.order.vOrderProductMonth, `?month=${props.month}&notCompflg=${props.notCompflg ? "1" : "0"}&supplierCode=${String(props.supplierCode || "")}`, arrPath[1])
    ...
    return <>
        {loading ?
            ...
            :
            <>
                <ViewItemList title={"[月別]仕入・納品一覧"} disableActionButton={true} ViewData={data} isLoading={loading} query={`?month=${props.month}&notCompflg=${props.notCompflg ? "1" : "0"}&supplierCode=${String(props.supplierCode || "")}`} />
