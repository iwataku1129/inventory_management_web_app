...
export const DashboardContentPageMain = () => {
    ...
    const handleChangeData = useCallback((result: any) => {
        setData({
            columns: [
                ...
                { title: 'ページ名', field: 'inventory_category', cellStyle: { textAlign: 'left', fontSize: "medium" } },
                { title: 'path(英語)', field: 'inventory_category_path', cellStyle: { textAlign: 'left', fontSize: "medium" } },
    ...
    useEffect(() => {
        ...
        if (inProgress === InteractionStatus.None) {
            ApiGet(apiPath.master.vInventCategory || "", ``, arrPath[1])
    ...
    return <>
        {loading ?
            ...
            :
            <ViewItemList title={`ページ一覧`} ViewData={data} isLoading={loading} />
        }
    </>;
};
