...
export const DashboardContentUserMain = () => {
    ...
    const handleChangeData = useCallback((result: any) => {
        setData({
            columns: [
                ...
                { title: 'ユーザ名', field: 'user_name', cellStyle: { textAlign: 'left', fontSize: "medium" } },
                { title: 'email', field: 'email', cellStyle: { textAlign: 'left', fontSize: "medium" } },
    ...
    useEffect(() => {
        ...
        if (inProgress === InteractionStatus.None) {
            ApiGet(apiPath.master.vUser || "", `all/`, arrPath[1])
    ...
    return <>
        {loading ?
            ...
            :
            <ViewItemList title={`ユーザ一覧`} ViewData={data} isLoading={loading} />
        }
    </>;
};
