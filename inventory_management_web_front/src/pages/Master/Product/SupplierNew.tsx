...
const CreateContent = (props: { routeCat: string, routeMethod: string, routePath: string }) => {
    ...
    const { control, handleSubmit, reset } = useForm<Inputs>({
        defaultValues: { supplier_code: "", corporation_code: "", Invoice_code: "", supplier_name: "", remarks: "" }
    })
        ...
        const getEditData = async () => {
            if (inProgress === InteractionStatus.None) {
                try {
                    ...
                    await ApiGet(apiPath.master.vSupplier, `id/${urlParamsId}`, props.routeCat)
    ...
    const handleClickDeleteButton = async () => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await ApiDelete(`${apiPath.master.supplier}/${urlParamsId}`, editMode.opentime || "", props.routeCat)
                    await Swal.fire({
                        title: '削除完了',
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data: Inputs) => {
        ...
        const values = {
            ...
            supplier_code: datas.supplier_code,
            corporation_code: datas.corporation_code,
            Invoice_code: datas.Invoice_code,
        ...
        if (props.routeMethod === "new") {
            try {
                const ret: any = await ApiPost(apiPath.master.supplier, values, props.routeCat)
                await Swal.fire({
                    title: '登録完了',
                    html: `「${values.supplier_name}」の登録が完了しました`,
                    timer: 5000,
                    icon: 'success'
                })
            ...
            try {
                await ApiPut(apiPath.master.supplier, editMode.opentime || "", values, props.routeCat)
                await Swal.fire({
                    title: '更新完了',
                    html: `「${values.supplier_name}」の更新が完了しました`,
                    timer: 5000,
                    icon: 'success'
                })
        ...
        <>
            {loading === true ?
                ...
                <Stack component="form" noValidate onSubmit={handleSubmit(onSubmit)}>
                    <div>
                        <TextInput
                            name="supplier_code" type={"text"} control={control} validationRule={validationRules.text} disabled={editMode.mode}
                            label={"取引先コード"} widthType={"small"} rows={1}
                        />
                        <TextInput
                            name="corporation_code" type={"text"} control={control} validationRule={{}} disabled={false}
                            label={"法人番号"} widthType={"small"} rows={1}
                        />
                        <TextInput
                            name="Invoice_code" type={"text"} control={control} validationRule={{}} disabled={false}
                            label={"適格事業者番号"} widthType={"small"} rows={1}
                        />
                    </div>
                    <div>
                        <TextInput
                            name="supplier_name" type={"text"} control={control} validationRule={validationRules.text} disabled={editMode.mode}
                            label={"取引先名"} widthType={"half"} rows={1}
                        />
                        <br />
                    </div>
                    <div>
...
export const SupplierNew = (props: { index: string }) => {
    ...
    return (
        <PageLayoutNew cardTitle="取引先管理" cardSubTitle="マスタ管理" cardText="" routePath={routePath} routeMethod={routeMethod}>
