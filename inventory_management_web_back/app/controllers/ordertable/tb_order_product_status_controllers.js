...
const tb_order_product_status = db.tb_order_product_status;
...
// Create and Save a new tb_order_product_status
exports.create = async (req, res) => {
    ...
    try {
        ...
        // Validate request
        if (!req.headers.invcat || !req.body.order_product_status) {
            await tx.rollback();
        ...
        // 重複チェック
        const conflict_check = await tb_order_product_status.findOne({ where: { [Op.and]: { inventory_category_id: inventory_category_id, order_product_status: req.body.order_product_status, del_flg: 0 } } })
        if (conflict_check) {
            await tx.rollback();
        ...
        // Set create data
        const tb_order_product_status_create = {
            inventory_category_id: inventory_category_id,
            order_product_status: req.body.order_product_status,
        ...
        // Save database
        const ret = await tb_order_product_status.create(tb_order_product_status_create, { transaction: tx })
...
// Update a tb_order_product_status by the id in the request
exports.update = async (req, res) => {
    ...
    try {
        ...
        // 登録実績・同期チェック
        const conflict_check = await tb_order_product_status.findOne({ where: { [Op.and]: { id: req.body.id, inventory_category_id: inventory_category_id, order_product_status: req.body.order_product_status, del_flg: 0 } }, order: [["updated_at", "desc"]] })
        if (!conflict_check) {
            await tx.rollback();
        ...
        const ret_update = await tb_order_product_status.update({
            //inventory_category_id: inventory_category_id,
            order_product_status: req.body.order_product_status,
            phase: req.body.phase,
            del_flg: 0,
...
// Delete a tb_order_product_status by the id in the request
exports.delete = async (req, res) => {
    ...
    try {
        ...
        // 登録実績チェック
        const conflict_check = await tb_order_product_status.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_id: inventory_category_id, del_flg: 0 } }, order: [["updated_at", "desc"]] })
        if (!conflict_check) {
            await tx.rollback();
            res.status(400).send({ message: "No registration" })
            return;
...
// Find a single tb_order_product_status with an id
exports.findOne = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_one = await tb_order_product_status.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_id: inventory_category_id, del_flg: 0 } } })
        if (get_data_one) {
...
// Find a single tb_order_product_status with an id
exports.findOnePhase = async (req, res) => {
    try {
        // Auth request
        if (!hasRequiredDelegatedPermissions(req.authInfo, authConfig.protectedRoutes.webapp.delegatedPermissions.scopes)) {
            res.status(403).send({ message: 'User does not have the required permissions' });
        ...
        // Validate request
        if (!req.params.phase || !req.headers.invcat) {
            res.status(400).send({ message: "Content can not be empty!" })
            return;
        }
        ...
        // Get datas
        const get_data_one = await tb_order_product_status.findOne({ where: { [Op.and]: { phase: { [Op.in]: opportunityPhase }, inventory_category_id: inventory_category_id, del_flg: 0 } } })
        if (get_data_one) {
            res.send(get_data_one)
            return;
        }
...
// Retrieve all tb_order_product_statuss from the database.
exports.findAll = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_all = await db.tb_order_product_status.findAll({ where: { inventory_category_id: inventory_category_id, del_flg: 0 }, order: [["id", "asc"]] })
