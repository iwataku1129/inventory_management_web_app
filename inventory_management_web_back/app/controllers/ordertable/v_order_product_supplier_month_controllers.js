...
const v_order_product_supplier_month = db.v_order_product_supplier_month;
...
// Find a single v_order_product_supplier_month with an id
exports.findOne = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_one = await v_order_product_supplier_month.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_path: req.headers.invcat } } })
        if (get_data_one) {
...
// Retrieve all v_order_product_supplier_months from the database.
exports.findAll = async (req, res) => {
    try {
        ...
        // Validate request
        if (!req.headers.invcat || !req.query.month) {
            res.status(400).send({ message: "Content can not be empty!" })
            return;
        }
        ...
        const supplierCode = req.query.supplierCode || null
        const notCompflg = req.query.notCompflg
        const query = { inventory_category_path: req.headers.invcat, deliver_month: month }
        if (supplierCode) { query["supplier_code"] = supplierCode }
        if (notCompflg === "1") { query["phase"] = { [Op.in]: ["order", "check"] } } else { query["phase"] = { [Op.in]: ["complete"] } }
        const get_data_all = await v_order_product_supplier_month.findAll({ where: { [Op.and]: query }, order: [["supplier_code", "asc"]], limit: 10000 })
