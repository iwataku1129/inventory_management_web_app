...
const v_report_stock_product_group = db.v_report_stock_product_group;
...
// Find a single v_report_stock_product_group with an id
exports.findOne = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_one = await v_report_stock_product_group.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_path: req.headers.invcat } } })
        if (get_data_one) {
...
// Retrieve all v_report_stock_product_groups from the database.
exports.findAll = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_all = await db.v_report_stock_product_group.findAll({ where: { inventory_category_path: req.headers.invcat }, order: [["id", "asc"]] })
...
exports.findAllQuery = async (req, res) => {
    try {
        ...
        const get_data_all = await sequelize.query(
            `select
                group_concat(distinct tables.order_id separator ',') as ids,
                ...
                group_concat(distinct tables.product_id separator ',') as product_ids,
                ...
                group_concat(distinct kamoku.product_kamoku_category separator ',') as product_kamoku_category,
                group_concat(distinct prd_cat1.product_category_1 separator ',') as product_category_1,
                group_concat(distinct prd_cat2.product_category_2 separator ',') as product_category_2,
                group_concat(distinct prd_cat3.product_category_3 separator ',') as product_category_3,
                ...
                sum(tables.order_price) / sum(tables.stock_cnt) as order_check_price_unit,
            ...
            from
                ...
                where orders.deliver_date <= '${date[0].replace("'", "\\'")}'
            union all
                select
                    orders.id as order_id,
                    '使用' as category_name,
                ...
                where day_rep.report_date <= '${date[0].replace("'", "\\'")}' and orders.deliver_date <= '${date[0].replace("'", "\\'")}'
            ) tables
            ...
            where
                inv_cat.inventory_category_path='${req.headers.invcat}' and order_sts.phase='complete'${productCodeQuery}
            group by
                ...
                tables.order_product_status_id
            order by
                product.product_code,
            ...
            `
            , {
                model: v_report_stock_product_group,
