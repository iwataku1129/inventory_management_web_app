...
const v_product_master_hist = db.v_product_master_hist;
...
// Find a single v_product_master_hist with an id
exports.findOne = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_one = await v_product_master_hist.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_path: req.headers.invcat, del_flg: 0 } } })
        if (get_data_one) {
...
// Retrieve all v_product_master_hists from the database.
exports.findAll = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_all = await db.v_product_master_hist.findAll({ where: { inventory_category_path: req.headers.invcat, del_flg: 0 }, order: [["id", "asc"]] })
