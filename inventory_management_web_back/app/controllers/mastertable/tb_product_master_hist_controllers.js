...
const tb_product_master_hist = db.tb_product_master_hist;
...
// Create and Save a new tb_product_master_hist
exports.create = async (req, res) => {
    ...
    try {
        ...
        // 重複チェック
        //const conflict_check = await tb_product_master_hist.findOne({ where: { [Op.and]: { inventory_category_id: inventory_category_id, product_code: req.body.product_code,product_name:req.body.product_name, del_flg: 0 } } })
        //if (conflict_check) {
        //    await tx.rollback();
        ...
        // Save database
        const ret = await tb_product_master_hist.create(tb_product_master_hist_create, { transaction: tx })
...
// Update a tb_product_master_hist by the id in the request
exports.update = async (req, res) => {
    ...
    try {
        ...
        // 登録実績 ・ 同期チェック
        const conflict_check = await tb_product_master_hist.findOne({ where: { [Op.and]: { id:req.body.id, inventory_category_id: inventory_category_id, del_flg: 0 } }, order: [["updated_at", "desc"]]})
        if (!conflict_check) {
            await tx.rollback();
        ...
        const ret_update = await tb_product_master_hist.update({
            //inventory_category_id: inventory_category_id,
...
// Delete a tb_product_master_hist by the id in the request
...
// Find a single tb_product_master_hist with an id
exports.findOne = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_one = await tb_product_master_hist.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_id: inventory_category_id, del_flg: 0 } } })
        if (get_data_one) {
...
// Retrieve all tb_product_master_hists from the database.
exports.findAll = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_all = await db.tb_product_master_hist.findAll({ where: { inventory_category_id: inventory_category_id, del_flg: 0 }, order: [["id", "asc"]] })
