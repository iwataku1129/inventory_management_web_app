...
const tb_product_kamoku_category = db.tb_product_kamoku_category;
...
// Create and Save a new tb_product_kamoku_category
exports.create = async (req, res) => {
    ...
    try {
        ...
        // Validate request
        if (!req.headers.invcat || !req.body.product_kamoku_category) {
            await tx.rollback();
        ...
        // 重複チェック
        const conflict_check = await tb_product_kamoku_category.findOne({ where: { [Op.and]: { inventory_category_id: inventory_category_id, product_kamoku_category: req.body.product_kamoku_category, del_flg: 0 } } })
        if (conflict_check) {
            await tx.rollback();
        ...
        // Set create data
        const tb_product_kamoku_category_create = {
            inventory_category_id: inventory_category_id,
            product_kamoku_category: req.body.product_kamoku_category,
            del_flg: 0,
        ...
        // Save database
        const ret = await tb_product_kamoku_category.create(tb_product_kamoku_category_create, { transaction: tx })
...
// Update a tb_product_kamoku_category by the id in the request
exports.update = async (req, res) => {
    ...
    try {
        ...
        // 登録実績・同期チェック
        const conflict_check = await tb_product_kamoku_category.findOne({ where: { [Op.and]: { id: req.body.id, inventory_category_id: inventory_category_id, product_kamoku_category: req.body.product_kamoku_category, del_flg: 0 } }, order: [["updated_at", "desc"]] })
        if (!conflict_check) {
            await tx.rollback();
        ...
        const ret_update = await tb_product_kamoku_category.update({
            //inventory_category_id: inventory_category_id,
            product_kamoku_category: req.body.product_kamoku_category,
            del_flg: 0,
            updated_by: userinfo.id,
...
// Delete a tb_product_kamoku_category by the id in the request
exports.delete = async (req, res) => {
    ...
    try {
        ...
        // 登録実績チェック
        const conflict_check = await tb_product_kamoku_category.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_id: inventory_category_id, del_flg: 0 } }, order: [["updated_at", "desc"]] })
        if (!conflict_check) {
            await tx.rollback();
            res.status(400).send({ message: "No registration" })
            return;
...
// Find a single tb_product_kamoku_category with an id
exports.findOne = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_one = await tb_product_kamoku_category.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_id: inventory_category_id, del_flg: 0 } } })
        if (get_data_one) {
...
// Retrieve all tb_product_kamoku_categorys from the database.
exports.findAll = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_all = await db.tb_product_kamoku_category.findAll({ where: { inventory_category_id: inventory_category_id, del_flg: 0 }, order: [["id", "asc"]] })
