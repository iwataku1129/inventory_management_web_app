...
const v_supplier_master = db.v_supplier_master;
...
// Find a single v_supplier_master with an id
exports.findOne = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_one = await v_supplier_master.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_path: req.headers.invcat, del_flg: 0 } } })
        if (get_data_one) {
...
// Retrieve all v_supplier_masters from the database.
exports.findAll = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_all = await db.v_supplier_master.findAll({ where: { inventory_category_path: req.headers.invcat, del_flg: 0 }, order: [["id", "asc"]] })
