...
const tb_day_report_product_category = db.tb_day_report_product_category;
...
// Create and Save a new tb_day_report_product_category
exports.create = async (req, res) => {
    ...
    try {
        ...
        // Validate request
        if (!req.headers.invcat || !req.body.day_report_product_category) {
            await tx.rollback();
        ...
        // 重複チェック
        const conflict_check = await tb_day_report_product_category.findOne({ where: { [Op.and]: { inventory_category_id: inventory_category_id, day_report_product_category: req.body.day_report_product_category, del_flg: 0 } } })
        if (conflict_check) {
            await tx.rollback();
        ...
        // Set create data
        const tb_day_report_product_category_create = {
            inventory_category_id: inventory_category_id,
            day_report_product_category: req.body.day_report_product_category,
            del_flg: 0,
        ...
        // Save database
        const ret = await tb_day_report_product_category.create(tb_day_report_product_category_create, { transaction: tx })
...
// Update a tb_day_report_product_category by the id in the request
exports.update = async (req, res) => {
    ...
    try {
        ...
        // 登録実績・同期チェック
        const conflict_check = await tb_day_report_product_category.findOne({ where: { [Op.and]: { id: req.body.id, inventory_category_id: inventory_category_id, day_report_product_category: req.body.day_report_product_category, del_flg: 0 } }, order: [["updated_at", "desc"]] })
        if (!conflict_check) {
            await tx.rollback();
        ...
        const ret_update = await tb_day_report_product_category.update({
            //inventory_category_id: inventory_category_id,
            day_report_product_category: req.body.day_report_product_category,
            del_flg: 0,
            updated_by: userinfo.id,
...
// Delete a tb_day_report_product_category by the id in the request
exports.delete = async (req, res) => {
    ...
    try {
        ...
        // 登録実績チェック
        const conflict_check = await tb_day_report_product_category.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_id: inventory_category_id, del_flg: 0 } }, order: [["updated_at", "desc"]] })
        if (!conflict_check) {
            await tx.rollback();
            res.status(400).send({ message: "No registration" })
            return;
...
// Find a single tb_day_report_product_category with an id
exports.findOne = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_one = await tb_day_report_product_category.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_id: inventory_category_id, del_flg: 0 } } })
        if (get_data_one) {
...
// Retrieve all tb_day_report_product_categorys from the database.
exports.findAll = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_all = await db.tb_day_report_product_category.findAll({ where: { inventory_category_id: inventory_category_id, del_flg: 0 }, order: [["id", "asc"]] })
