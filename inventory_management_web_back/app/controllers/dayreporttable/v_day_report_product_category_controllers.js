...
const v_day_report_product_category = db.v_day_report_product_category;
...
// Find a single v_day_report_product_category with an id
exports.findOne = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_one = await v_day_report_product_category.findOne({ where: { [Op.and]: { id: req.params.id, inventory_category_path: req.headers.invcat, del_flg: 0 } } })
        if (get_data_one) {
...
// Retrieve all v_day_report_product_categorys from the database.
exports.findAll = async (req, res) => {
    try {
        ...
        // Get datas
        const get_data_all = await db.v_day_report_product_category.findAll({ where: { inventory_category_path: req.headers.invcat, del_flg: 0 }, order: [["id", "asc"]] })
